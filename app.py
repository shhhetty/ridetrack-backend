...
# Step 2: Define global variables and base configurations
basedir = os.path.abspath(os.path.dirname(__file__))

# Step 3: Initialize extensions WITHOUT an app instance yet
db = SQLAlchemy()
jwt = JWTManager()
socketio = SocketIO()

# Step 4: Define all your database models
...
# Step 5: Create a function to build the Flask app
def create_app():
    # Configure the app to serve static files from the 'frontend' folder
    ...
    db.init_app(app)
    jwt.init_app(app)
    # Let SocketIO handle its own CORS. No other CORS library is needed.
    ...
    def register():
        ...
        if not data or not data.get('username') or not data.get('email') or not data.get('password'): return jsonify({"error": "Missing required fields"}), 400
        if User.query.filter_by(email=data.get('email')).first() or User.query.filter_by(username=data.get('username')).first(): return jsonify({"error": "Email or username already in use"}), 409
    ...
    def login():
        data = request.get_json()
        if not data or not data.get('email') or not data.get('password'): return jsonify({"error": "Missing email or password"}), 400
    ...
    def profile():
        ...
        user_data = {"id": user.id, "username": user.username, "email": user.email, "city": user.city, "bike_model": user.bike_model, "bio": user.bio, "riding_style": user.riding_style, "joined_groups": joined_groups_ids}
    ...
    def get_group_details(group_id):
        ...
        group_data = {"id": group.id, "name": group.name, "description": group.description, "creator_username": group.creator.username if group.creator else "Unknown", "members": members_list, "active_ride_id": active_ride.id if active_ride else None}
    ...
    @app.route('/api/connections/send/<int:receiver_id>', methods=['POST'])
    @jwt_required()
    ...
    @app.route('/api/connections/accept/<int:requester_id>', methods=['POST'])
    @jwt_required()
    ...
    @app.route('/api/groups/<int:group_id>/start_ride', methods=['POST'])
    @jwt_required()
    ...
    @app.route('/api/groups/<int:group_id>/end_ride', methods=['POST'])
    @jwt_required()
    ...
    def handle_location_update(data):
        ...
        if not room or not user_info or not location: return
        socketio.emit('new_location', {'user': user_info, 'location': location}, to=room, include_self=False)
...
if __name__ == '__main__':
    app = create_app()
    # Use socketio.run to start the server on the correct port
